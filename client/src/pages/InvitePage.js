...
const InvitePage = (props) => {
	...
	async function sendInviteMail(dest, token) {
		console.log("invite sending fnction called...")
		const req_body = {
			source: "support@cyethack.com",
			destinations: [dest],
			subject: "Astra Onboarding Invitation",
			body: "Hello from cyethack!",
			html: "<h1>Visit http://localhost:3001/activateuser?token=" + token + " to initiate onboarding!<h1>",
		}
		const response = await fetch('https://qet85fubbi.execute-api.ap-south-1.amazonaws.com/dev/sendemail', {
			method: 'POST',
			// credentials: 'include',
			headers: {
	...
	async function inviteUser(event) {
		...
		if (data.status == "ok") {
			...
			await sendInviteMail(email, data.pending_user_id)
		} else {
